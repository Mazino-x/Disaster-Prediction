        ...
        const fetchPredictions = async () => {
            ...
            try {
                ...
                try {
                    ...
                    if (!eqResponse.ok) {
                        const message = eqData.error || 'Earthquake prediction failed';
                    ...
                    setEarthquake('Error');
                    setError(prev => prev || 'Earthquake prediction unavailable.');
                }
                ...
                try {
                    ...
                    if (!tsunamiResponse.ok) {
                        const message = tsunamiData.error || 'Tsunami prediction failed';
                        throw new Error(message);
                    }
                    ...
                    setTsunami('Error');
                    setError(prev => prev || 'Tsunami prediction unavailable.');
                }
            } catch (e) {
    ...
    useEffect(() => {
        ...
        const loadWeather = async () => {
            try {
                ...
                setLocalTime(data.current_weather.time);
    ...
    return (
        <div className="dashboardContainer">
            <div className="dashboardHeader">
                ...
                {localTime && <p className="coordinates"><strong>Local time:</strong> {new Date(localTime).toLocaleString()}</p>}
                {localWeather && <p className="coordinates"><strong>Current weather:</strong> {localWeather.description}, {localWeather.temperature}°C</p>}
                {weatherError && <p className="coordinates">{weatherError}</p>}
            </div>
            ...
            <div className="predictionsGrid">
                <div className="predictionCard earthquakeCard">
                    ...
                    <div className="cardDetails">
                        ...
                        {localTime && <p><strong>Current local time:</strong> {new Date(localTime).toLocaleString()}</p>}
                        {localWeather && <p><strong>Weather:</strong> {localWeather.description}, {localWeather.temperature}°C</p>}
